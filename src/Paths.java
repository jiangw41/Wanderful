...
public class Paths {
	...
	// The 2 Choices holds a list of points (List of directions) going from one
	...
	public Paths(Intersection point, Intersection[] arr, double distanceGoal) {
		...
		ArrayList<Intersection> choicesPrint = new ArrayList<Intersection>();

		choices.add(point);
		ArrayList<Intersection> choices2 = new ArrayList<Intersection>();

		ArrayList<Intersection> choices2Print = new ArrayList<Intersection>();
		choices2.add(point);
		...
		// Calls paths method below with the Choice List (0 for Route 1, 1 for Route 2),
		// PointInfo, dataset,
		// distanceGoal and a number referring to the two points the current point is
		// closest to
		...
		for (Intersection I : this.Choices) {
			if (!temp.equals(I.toString().split(" ")[1])) {
				temp = I.toString().split(" ")[1];
				choicesPrint.add(I);
			}
		...
		System.out.println("Option 1: " + choicesPrint);
		// Goes solely to the 2nd point in the NearbyPoints list
		paths(choices2, PI2, arr, distanceGoal, 1);
		this.Choices2 = choices2;
		temp = "";

		for (Intersection J : this.Choices2) {
			if (!temp.equals(J.toString().split(" ")[1])) {
				temp = J.toString().split(" ")[1];
				choices2Print.add(J);
			}

		}
		System.out.println("Option 2: " + choices2Print);

	}

	// paths method given a list of points to update recursively, PointInfo
	// containing current point and choices,
	// dataset array, distance Goal and a number referring to the two points the
	// current point is closest to
	public void paths(ArrayList<Intersection> choices, PointInfo PI, Intersection[] arr, double distanceGoal, int i) {

		// NearbyPoints method with initial point and dataset to generate a list holding
		// two nearby points
		NearbyPoints Points = new NearbyPoints(PI.getPoint(), arr);

		// Holds onto list of points containing 2 nearby points from the NearbyPoints
		// method above
